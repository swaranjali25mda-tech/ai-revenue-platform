...
def generate_pdf():
    ...
    elements.append(
        Paragraph(
            f"Highest Revenue: ₹{round(max_value, 2)}",
            styles["Normal"]
        )
    )
...
def model_performance_page(
    ...
    plt.savefig(
        "static/images/model_performance.png"
    )
...
def ai_insights_page(
    ...
    if last_rev > mean_rev:
        insights.append(
            "Revenue is currently above historical average."
        )
    else:
        insights.append(
            "Revenue is below historical average."
        )

    if volatility > mean_rev * 0.2:
        insights.append(
            "Revenue shows high volatility. "
